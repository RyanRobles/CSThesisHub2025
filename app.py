...
def search():
    ...
    def safe_join_fields(result, fields):
    ...
    for result in metadata_results:
        combined_text = ' '.join([
        result.get('title') or '',
        result.get('authors') or '',
        #result.get('keywords') or '',
        result.get('school') or ''
    ])
    ...
    try:
        ...
        if user:
            ...
            try:
                ...
                logging.error(f"Error creating Google account: {e}")
...
def browse_theses():
    ...
    # Sorting
    if sort_by == 'recent':
        query += " ORDER BY pt.year_made DESC"
    elif sort_by == 'oldest':
        query += " ORDER BY pt.year_made ASC"
    elif sort_by == 'title':
...
def admin_upload():
    ...
    try:
        ...
        # Process CamScanner PDF if needed
        if filename.lower().endswith('.pdf'):
            # Check if this looks like a CamScanner PDF (you might need to adjust this)
            with open(filepath, 'rb') as f:
                if b'CamScanner' in f.read(1024):
                    # Process the CamScanner PDF
                    temp_dir = tempfile.mkdtemp()
                    processed_pages = process_camscanner_pdf(filepath, os.path.join(temp_dir, 'processed'))
        ...
        # Extract metadata (will now get keywords from abstract)
...
def review_submission(submission_id):
    ...
    if submission.get('extracted_text'):
        split_pages = submission['extracted_text'].split('\n\n', 1)
        title_text = split_pages[0] if len(split_pages) >= 1 else ''
        abstract_text = split_pages[1] if len(split_pages) >= 2 else ''
    else:
        title_text = ''
        abstract_text = ''
...
def edit_published_thesis(thesis_id):
    ...
    if request.method == 'POST':
        ...
        year_made = request.form.get('year_made', '').strip()

        # Validate required fields
        if not all([title, authors, school, year_made]):
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET title = %s, authors = %s, school = %s,
                    year_made = %s
                WHERE id = %s
            """, (title, authors, school, year_made, thesis_id))

            # Insert into version history
            cursor.execute("""
                INSERT INTO thesis_versions
                (thesis_id, edited_title, edited_authors,
                 edited_school, edited_year_made,
                 notes, edited_by)
            ...
            if thesis.get('submission_id'):
                cursor.execute("""
                    ...
                    SET title = %s, authors = %s, school = %s,
                        year_made = %s
                    WHERE id = %s
                """, (title, authors, school, year_made, thesis['submission_id']))
            ...
            app.logger.error(f"Error editing thesis {thesis_id}: {str(e)}")
